...
def procesar():
    ...
    if data and lib:
        ...
        print(f"{'AÑO':<6} | {'GINI ORIGINAL':<15} | {'ENTERO (C)':<10} | {'SUMA +1 (C)':<10}")
        print("-" * 55)
        
        for entry in top_5:
            v_float = entry.get('value')
            anio = entry.get('date')
            g_int = lib.float_to_int(v_float)
            g_final = lib.sumar_uno(g_int)
            print(f"{anio:<6} | {v_float:<15.2f} | {g_int:<10} | {g_final:<10}")
